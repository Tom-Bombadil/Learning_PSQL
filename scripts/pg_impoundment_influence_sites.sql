  ...
  FROM (
	...
	FROM (
      ...
      WHERE featureid IN (
        ...
        FROM gis.detailed_flowlines, public.locations_test 
        WHERE ST_Intersects(ST_Buffer(public.locations_test.geom::geography, 75), gis.detailed_flowlines.geom::geography)
      )
	) as monkey
  ) as dumped
) AS simple;
...
SELECT pt_id, ln_id
  FROM gis.snapped_locations, gis.impoundment_zones
  WHERE ST_Intersects(ST_Buffer(snapped_locations.geom::geography, 1)  , impoundment_zones.geom);
